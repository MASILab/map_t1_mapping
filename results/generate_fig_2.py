...
cbar = fig.colorbar(im, ax=axes[0,0], cax=cax, location='left')
...
cbar = fig.colorbar(im, ax=axes[0,1], cax=cax, location='left')
cbar.ax.set_xlabel('s')
...
cbar = fig.colorbar(im, ax=axes[1,0], cax=cax, location='left')
cbar.ax.set_xlabel('s')

im = axes[1,1].imshow(t1_std_slice, cmap='viridis', vmin=0, vmax=1)
axes[1,1].set_title('Standard Deviation of $T_1$')
axes[1,1].set_axis_off()
cax = axes[1,1].inset_axes([-0.1, 0, 0.05, 1])
cbar = fig.colorbar(im, ax=axes[1,1], cax=cax, location='left')
cbar.ax.set_xlabel('s')
